...
-(void)setupView {
    ...
    [self addSubview:self.righTwoButton];
    [self addSubview:self.righThreeButton];
...
-(void)updateFrame {
    ...
    self.righTwoButton.frame = CGRectMake(kWRScreenWidth - buttonWidth*2 - margin, top, buttonWidth, buttonHeight);
    self.righThreeButton.frame = CGRectMake(kWRScreenWidth - buttonWidth*3 - margin, top, buttonWidth, buttonHeight);
...
-(void)clickRightTwo {
    if (self.onClickRightTwoButton) {
        self.onClickRightTwoButton();
    }
}
-(void)clickRightThree {
    if (self.onClickRightThreeButton) {
        self.onClickRightThreeButton();
    }
}
...
- (void)wr_setTintColor:(UIColor *)color {
    ...
    [self.righTwoButton setTitleColor:color forState:UIControlStateNormal];
    [self.righThreeButton setTitleColor:color forState:UIControlStateNormal];
...
- (void)wr_setRightTwoButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted title:(NSString *)title titleColor:(UIColor *)titleColor {
    self.righTwoButton.hidden = NO;
    [self.righTwoButton setImage:normal forState:UIControlStateNormal];
    [self.righTwoButton setImage:highlighted forState:UIControlStateHighlighted];
    [self.righTwoButton setTitle:title forState:UIControlStateNormal];
    [self.righTwoButton setTitleColor:titleColor forState:UIControlStateNormal];
}

- (void)wr_setRightThreeButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted title:(NSString *)title titleColor:(UIColor *)titleColor {
    self.righThreeButton.hidden = NO;
    [self.righThreeButton setImage:normal forState:UIControlStateNormal];
    [self.righThreeButton setImage:highlighted forState:UIControlStateHighlighted];
    [self.righThreeButton setTitle:title forState:UIControlStateNormal];
    [self.righThreeButton setTitleColor:titleColor forState:UIControlStateNormal];
}
...
- (void)wr_setRightTwoButtonWithImage:(UIImage *)image{
    [self wr_setRightTwoButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}

- (void)wr_setRightThreeButtonWithImage:(UIImage *)image{
    [self wr_setRightThreeButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
...
-(UIButton *)righTwoButton{
    if (!_righTwoButton) {
        _righTwoButton = [[UIButton alloc] init];
        [_righTwoButton addTarget:self action:@selector(clickRightTwo) forControlEvents:UIControlEventTouchUpInside];
        _righTwoButton.imageView.contentMode = UIViewContentModeCenter;
        _righTwoButton.hidden = YES;
    }
    return _righTwoButton;
}
-(UIButton *)righThreeButton{
    if (!_righThreeButton) {
        _righThreeButton = [[UIButton alloc] init];
        [_righThreeButton addTarget:self action:@selector(clickRightThree) forControlEvents:UIControlEventTouchUpInside];
        _righThreeButton.imageView.contentMode = UIViewContentModeCenter;
        _righThreeButton.hidden = YES;
    }
    return _righThreeButton;
}
...
//    BOOL isIPhoneX = [platform isEqualToString:@"iPhone10,3"] || [platform isEqualToString:@"iPhone10,6"];
    CGFloat KSScreenH = [UIScreen mainScreen].bounds.size.height;
    BOOL isIPhoneX = KSScreenH >= 812 ? YES : NO;
    return isIPhoneX;
}
